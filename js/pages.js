...
export async function doAuth() {
  ...
  try {
    ...
    var ALLOWED=['cyb@qq.com','cyb13579_2023@qq.com','1870403731@qq.com'];var userEmail=supabase.getUser()?.email||'';if(ALLOWED.indexOf(userEmail)===-1){await supabase.signOut();updateNav();toast('你没有权限登录');return}
    toast('登录成功');
    updateNav();
    location.hash = '#/';
  } catch (e) {
