...
export async function renderVideos() {
  ...
  document.getElementById('main').innerHTML = '<a href="#/" class="back-link">← 返回</a>' +
    '<div class="detail-card"><div class="detail-actions" style="margin-bottom:16px"><h1 class="detail-title" style="margin-bottom:0;flex:1">视频分享</h1>' +
    (supabase.getUser() ? '<button class="btn btn-primary btn-small" onclick="location.hash=\'#/video-upload\'">+ 分享视频</button>' : '') +
    '</div></div>' +
    '<div class="paper-grid" id="videoGrid"></div>';

  try {
    var videos = await supabase.query('papers', { where: { media_type: 'video' }, order: 'created_at.desc', limit: 50 });
    var grid = document.getElementById('videoGrid');
    if (!videos || !videos.length) {
      grid.innerHTML = '<div class="empty-state"><div class="icon">—</div><p>还没有视频分享</p></div>';
      return;
    }
    grid.innerHTML = videos.map(function(v) {
      var platform = detectPlatform(v.file_path || '');
      return '<div class="paper-card" onclick="location.hash=\'#/video-detail/' + v.id + '\'">' +
        '<div class="paper-title">' + esc(v.title || '') + '</div>' +
        '<div class="paper-tags"><span class="tag tag-subject">' + (platform||'视频') + '</span>' +
        '<span class="tag tag-year">' + esc(v.tags || '') + '</span></div>' +
        '<div class="paper-footer"><span>' + timeAgo(v.created_at) + '</span><span>' + esc(v.user_email || '') + '</span></div></div>';
    }).join('');
  } catch (e) { /* ignore */ }
}

export function renderVideoUpload() {
  if (!supabase.getUser()) { location.hash = '#/login'; return; }
  document.getElementById('hero').style.display = 'none';
  document.getElementById('main').innerHTML =
    '<a href="#/videos" class="back-link">← 返回</a>' +
    '<div class="detail-card"><h1 class="detail-title">分享视频</h1>' +
    '<div class="form-group"><label>标题</label><input type="text" id="vidTitle" placeholder="视频标题" maxlength="100"></div>' +
    '<div class="form-group"><label>视频链接</label><input type="text" id="vidUrl" placeholder="B站/YouTube/视频直链"></div>' +
    '<div class="form-group"><label>标签（逗号分隔）</label><input type="text" id="vidTags" placeholder="公开课,专业"></div>' +
    '<div class="form-group"><label>描述（可选）</label><textarea id="vidDesc" rows="2" placeholder="简要介绍"></textarea></div>' +
    '<div class="modal-actions">' +
    '<button class="btn btn-ghost" onclick="location.hash=\'#/videos\'">取消</button>' +
    '<button class="btn btn-primary" onclick="window._uploadVideo()">分享</button></div></div>';
}
...
export async function uploadVideo() {
  ...
  var tags = document.getElementById('vidTags').value.trim();
  var desc = document.getElementById('vidDesc').value.trim();
  if (!title || !url) { toast('请填写标题和链接'); return; }
  try {
    await supabase.create('papers', {
      title: title, subject: '视频', year: new Date().getFullYear(),
      tags: tags, description: desc, file_path: url,
      file_type: 'video', media_type: 'video', file_size: 0,
      user_id: user.id, user_email: user.email,
      downloads: 0, avg_rating: 0, rating_count: 0
    });
    toast('分享成功'); location.hash = '#/videos';
  } catch (e) { toast('分享失败: ' + e.message); }
}

export async function renderVideoDetail(id) {
  if (!id) { location.hash = '#/videos'; return; }
  try {
    var v = await supabase.get('papers', id);
    if (!v) { toast('视频不存在'); location.hash = '#/videos'; return; }
    var url = v.file_path || ''; var embed = getVideoEmbed(url);
    document.getElementById('main').innerHTML =
      '<a href="#/videos" class="back-link">← 返回</a>' +
      '<div class="detail-card"><h1 class="detail-title">' + esc(v.title||'') + '</h1>' +
      '<div class="paper-tags" style="margin-bottom:12px"><span class="tag tag-subject">' + (detectPlatform(url)||'视频') + '</span>' +
      (v.tags?'<span class="tag tag-year">'+esc(v.tags)+'</span>':'') +
      '<span style="font-size:0.82rem;color:var(--text-muted)">'+esc(v.user_email||'')+' · '+timeAgo(v.created_at)+'</span></div>' +
      (v.description?'<div style="margin-bottom:12px;color:var(--text-secondary)">'+esc(v.description)+'</div>':'') +
      (embed?'<div style="position:relative;padding-bottom:56.25%;height:0;overflow:hidden;max-width:100%;margin-bottom:12px">'+embed+'</div>':
       '<p><a href="'+esc(url)+'" target="_blank">打开视频链接</a></p>') +
      '</div>';
  } catch (e) { toast('加载失败'); location.hash = '#/videos'; }
}

function detectPlatform(url) {
  if (!url) return '';
  if (url.indexOf('bilibili.com')>-1||url.indexOf('b23.tv')>-1) return 'B站';
  if (url.indexOf('youtube.com')>-1||url.indexOf('youtu.be')>-1) return 'YouTube';
  if (url.indexOf('mp4')>-1||url.indexOf('.mov')>-1||url.indexOf('.webm')>-1) return '视频';
  return '链接';
}
...
function getVideoEmbed(url) {
  if (!url) return '';
  // Bilibili
  var bv = url.match(/BV\w+/i)||url.match(/av\d+/i);
  if (bv) return '<iframe src="https://player.bilibili.com/player.html?bvid='+bv[0]+'&page=1" scrolling="no" border="0" frameborder="no" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe>';
  // YouTube
